...
class Kalman:
    def __init__(self,nState, nMeasure):
        ...
        self.transition_mat = np.zeros((nState, nState))        # 状态转移矩阵 A
        self.measure_mat = np.zeros((nMeasure, nState))           # 观测矩阵 H
        self.state_pre = np.zeros((nState, 1))             # 状态的先验估计X'(k)
        self.state_post = np.zeros((nState, 1))            # 校正状态 X(k)，其本质就是前一时刻的状态
        self.measurement = np.zeros((nMeasure, 1))           # 观测变量 Z(k)
        self.measure_noise_cov = np.zeros((nMeasure, nMeasure))     # 测量噪声协方差矩阵 R
        self.process_noise_cov = np.zeros((nState, nState))     # 过程噪声协方差矩阵 Q
        self.err_cov_post = np.zeros((nState, nState))          # 误差协方差 P
        self.err_cov_pre = np.zeros((nState, nState))           # 前一时刻的误差协方差 P(k-1)
        self.kalman_gain = np.zeros((nState, nMeasure))           # Kalman增益
